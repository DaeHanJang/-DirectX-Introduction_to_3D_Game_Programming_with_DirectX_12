...
GeometryGenerator::MeshData GeometryGenerator::CreateGeosphere(float radius, uint32 numSubdivisions) {
	...
	for (uint32 i = 0; i < 12; ++i) meshData.Vertices[i].Position = pos[i];
...
GeometryGenerator::MeshData GeometryGenerator::CreateCylinder(
	...
	//한 층 위로 더미로 올라갈 때의 반지름 변화량을 구한다.
	...
	for (uint32 i = 0; i < ringCount; ++i) {
		...
		for (uint32 j = 0; j <= sliceCount; ++j) {
			...
			//도입했다. 이렇게 하면 종접선(bitangent)이 텍스처 좌표 v 
			//성분과 동일한 방향이 된다.
	...
	//다르므로 서로 다른 정점으로 간주해야 함, 이를 위해 고리의 정점 
	//개수에 1을 더한다.
	uint32 ringVertexCount = sliceCount + 1;
...
void GeometryGenerator::BuildCylinderTopCap(float bottomRadius, float topRadius, float height,
	...
	//위 마개 정점들을 최상단 고리 정점들과 위치가 같지만 
	//텍스처 좌표의 법선이 다르므로 이처럼 따로 추가해야 한다.
	for (uint32 i = 0; i <= sliceCount; ++i) {
