...
void StencilApp::Draw(const GameTimer& gt)
{
	...
	//불투명한 항목(바닥, 벽, 두개골)을 그린다.
	auto passCB = mCurrFrameResource->PassCB->Resource();
	mCommandList->SetGraphicsRootConstantBufferView(2, passCB->GetGPUVirtualAddress());
    DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::Opaque]);
	
	//가시적 거울 픽셀들을 스텐실 버퍼에 1로 표시해 둔다.
	mCommandList->OMSetStencilRef(1);
	mCommandList->SetPipelineState(mPSOs["markStencilMirrors"].Get());
	DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::Mirrors]);

	//반사상을 거울 영역에만 그린다(스텐실 버퍼 항목이 1인 픽셀들만 
	//그려지게 한다).
	//이전과는 다른 패스별 상수 버퍼를 지정해야 함을 주목하기 바란다.
	//거울 평면에 대해 반사된 광원 설정을 담은 패스별 상수 버퍼를 
	//지정한다.
	mCommandList->SetGraphicsRootConstantBufferView(2, passCB->GetGPUVirtualAddress() + 1 * passCBByteSize);
	mCommandList->SetPipelineState(mPSOs["drawStencilReflections"].Get());
	DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::Reflected]);

	//기본 패스별 상수들과 스텐실 기준값을 복원한다.
	mCommandList->SetGraphicsRootConstantBufferView(2, passCB->GetGPUVirtualAddress());
	mCommandList->OMSetStencilRef(0);

	//반사상이 비치도록 투명도 혼합을 적용해서 거울을 그린다.
...
void StencilApp::UpdateReflectedPassCB(const GameTimer& gt)
{
	...
	//반사된 조명 설정을 패스별 상수 버퍼 색인 1에 저장한다.
	auto currPassCB = mCurrFrameResource->PassCB.get();
...
void StencilApp::BuildPSOs()
{
	...
	//투명도 혼합을 적용해서 그림자를 그릴 것이므로, 기존의 투명도 혼합 
	//설정을 바탕으로 삼아서 이 예제에 필요한 부분만 변경한다.
	...
	//후면 다각형들은 렌더링하지 않으므로 다음 설정들은 중요하지 않다.
