...
void LandAndWavesApp::Draw(const GameTimer& gt) {
	...
	else ThrowIfFailed(mCommandList->Reset(cmdListAlloc.Get(), mPSOs["opaque"].Get()));
...
void LandAndWavesApp::UpdateWaves(const GameTimer& gt) {
	...
	//파도 렌더 항목의 동적 VB를 현재 프레임 VB로 설정한다.
...
void LandAndWavesApp::BuildLandGeometry() {
	...
	//그 높이에 기초해서 정저의 색상도 적절히 설정한다. 이를 통해서 
	//모래색의 해변과 녹색의 언덕, 그리고 흰 눈이 엎인 봉우리 같은 모습이 
	//만들어진다.
	//
...
void LandAndWavesApp::BuildWavesGeometryBuffers() {
	std::vector<std::uint16_t> indices(3 * mWaves->TriangleCount()); //면(삼각형)당 색인 3개
...
XMFLOAT3 LandAndWavesApp::GetHillsNormal(float x, float z) const {
	XMFLOAT3 n(-0.03f * z * cosf(0.1f * x) - 0.3f * cosf(0.1f * z), 1.f, -0.3f * sinf(0.1f * x) + 0.03f * x * sinf(0.1f * z));
