...
//이런 구조의 구체적인 구성은 응용 프로그램마다 다를 수 있다.
struct RenderItem {
    RenderItem() = default;

    //세계 공간을 기준으로 물체의 국소 공간을 서술하는 세계 행렬.
    //이 행렬은 세계 공간 안에서의 물체의 위치와 방향, 크기를 
    //결정한다.
    XMFLOAT4X4 World = MathHelper::Identity4x4();

    //물체의 자료가 변해서 상수 버퍼를 갱신해야 하는지의 여부를 
    //뜻하는 더러움 플래그. FrameResource마다 물체의 cbuffer가 
    //있으므로, FrameResource마다 갱신을 적용해야 한다. 따라서, 
    //물체의 자료를 수정할 때에는 반드시 
    //NumFrameDirty = gNumFrameResources
...
void ShapesApp::Update(const GameTimer& gt) {
    ...
    //다 처리하지 않았으면 GPU가 이 울타리 지점까지의 명령들을 처리할 
    //때까지 기다린다.
...
void ShapesApp::Draw(const GameTimer& gt) {
    ...
    //현재 울타리 지점까지의 명령들을 표시하도록 울타리 값(Fence 멤버)을 
    //전진시킨다.
    mCurrFrameResource->Fence = ++mCurrentFence;

    //새 울티리 지점을 설정하는 명령(Signal)을 명령 대기열에 추가한다.
...
void ShapesApp::BuildDescriptorHeaps() {
    ...
    //각 프레이 자원의 물체마다 하나씩 CBV 서술자가 필요하다.
...
void ShapesApp::BuildConstantBufferViews() {
    ...
    for (int frameIndex = 0; frameIndex < gNumFrameResources; ++frameIndex) {
        ...
        for (UINT i = 0; i < objCount; ++i) {
            ...
            //현재 버퍼에서 i번째 상수 버퍼의 오프셋을 가상 주소에 더한다.
