...
private:
	...
	void AnimateMaterials(const GameTimer& gt); //함수 추가
	void UpdateObjectCBs(const GameTimer& gt);
	void UpdateMaterialCBs(const GameTimer& gt); //함수 추가
...
private:
	...
	UINT mCbvSrvDescriptorSize = 0; //변수 추가
	...
	std::unordered_map<std::string, std::unique_ptr<Material>> mMaterials; //맵 추가
	std::unordered_map<std::string, std::unique_ptr<Texture>> mTextures;
	std::unordered_map<std::string, ComPtr<ID3DBlob>> mShaders;

	std::vector<D3D12_INPUT_ELEMENT_DESC> mInputLayout;

	ComPtr<ID3D12PipelineState> mOpaquePSO = nullptr; //변수 추가
...
void LitColumnsApp::Update(const GameTimer& gt) {
	...
	AnimateMaterials(gt); //추가 부분
	UpdateObjectCBs(gt);
	UpdateMaterialCBs(gt); //추가 부분
	UpdateMainPassCB(gt);
}
...
void LitColumnsApp::BuildRenderItems() {
	...
	boxRitem->Mat = mMaterials["stone0"].get(); //추가 부분
	...
	gridRitem->Mat = mMaterials["tile0"].get(); //추가 부분
	...
	skullRitem->Mat = mMaterials["skullMat"].get(); //추가 부분
	...
	for (int i = 0; i < 5; ++i) {
		...
		leftCylRitem->Mat = mMaterials["bricks0"].get(); //추가 부분
		...
		rightCylRitem->Mat = mMaterials["bricks0"].get(); //추가 부분
		...
		leftSphereRitem->Mat = mMaterials["stone0"].get(); //추가 부분
		...
		rightSphereRitem->Mat = mMaterials["stone0"].get(); //추가 부분
